...
def setup_probs(setup_cfg):
    ...
    c1_mat = c_min + (c_max - c_min) * jnp.array(np.random.rand(N, split))
    c2_mat = split_factor * (c_min + (c_max - c_min) * jnp.array(np.random.rand(N, n_orig - split)))
    ...
    gd_setup_script(b_mat, P, output_filename)
